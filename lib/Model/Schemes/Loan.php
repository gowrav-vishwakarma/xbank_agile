...
class Model_Schemes_Loan extends Model_Schemes {
    ...
    function init(){
 	...
 	function newSchemeForm(&$owner){
        $owner->add('H3')->set('DDS COPY DDS COPY DDS COPY DDS COPY DDS COPY');
		$form = $owner->add('Form');
        $form->addClass('stacked atk-row');
        $form->template->trySet('fieldset','span6');
        $form->setModel('Schemes_DDS',array('Name','MinLimit','MaxLimit','Interest','AccountOpenningCommission','balance_sheet_id','MaturityPeriod','SchemePoints','SchemeGroup','ActiveStatus'));
    ...
    function editSchemeForm(&$owner){
        ...
        $form->addClass('stacked atk-row');
        $form->template->trySet('fieldset','span6');
        $form->setModel($this,array('Name','MinLimit','MaxLimit','Interest','AccountOpenningCommission','balance_sheet_id','MaturityPeriod','SchemePoints','SchemeGroup','ActiveStatus'));
        $form->add('Order')->move($form->addSeparator('span6'),'after','AccountOpenningCommission')->now();
